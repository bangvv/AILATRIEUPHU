...
#define INSERT_QUERY = "INSERT INTO scores (username, score, time) VALUES (?,?,?)";
...
int handle_play_game(Message msg, int conn_fd, Question *questions, int level, int isPlayPvP);
int handle_play_alone(int);
int handle_play_pvp(int);
void write_score_to_db(int conn_fd, char msg_data[BUFF_SIZE]);
void get_score_from_db(int conn_fd, char user_name[BUFF_SIZE]);
...
recvLabel:
    ...
    case OVER_TIME:
    case STOP_GAME:
      handle_play_game(msg, conn_fd, &questions, level, 0);
      return 0;
    case CHOICE_ANSWER:
      re = handle_play_game(msg, conn_fd, &questions, level, 0);
      if(re == 0) continue;
      return 0;
    case FIFTY_FIFTY:
    case CALL_PHONE:
...
{
  Message msg, tmp;
  int is_found = 0, index_in_room = -1, index_doi_thu_in_room = -1, is_me_win = -1, re, thoi_gian_tra_loi = 0, dap_an;
  Room *room;
  char str[BUFF_SIZE];
...
recvLabel2:
      ...
      case CALL_PHONE:
      case VOTE:
        handle_play_game(msg, conn_fd, &(room->questions[index_in_room]), room->index_current_question[index_in_room], 1);
        msg.type = -1;
        goto recvLabel2;
      case CHANGE_QUESTION:
        handle_play_game(msg, conn_fd, &(room->questions[index_in_room]), room->index_current_question[index_in_room], 1);
...
void write_score_to_db(int conn_fd, char data[BUFF_SIZE]) {

  char query[1000], id[20], user[50], score[50], time[50];
  ...
  printf("user:%s\t,score:%s\t,time:%s\n",user, score, time);

  sprintf(query, "INSERT INTO scores (username, score, time) VALUES ('%s','%s','%s')",user, score, time);
  execute_query(query);

  printf("Data inserted successfully\n");
}

void get_score_from_db(int conn_fd, char user_name[]) {
  printf("START\n");
  MYSQL_RES *res;
  MYSQL_ROW row;
  ...
  sprintf(query,"SELECT COUNT(*) FROM scores WHERE username = '%s'", user_name);
  execute_query(query);
  res = mysql_store_result(conn);
  row = mysql_fetch_row(res);
  int length_db = atoi(row[0]);
  int index = 0;
  printf("length:%d\n", length_db);
  if (length_db == 0) return;
  sprintf(query, "SELECT id, username, score, time FROM scores WHERE username = '%s' LIMIT 10", user_name);
  execute_query(query);
  res = mysql_store_result(conn);
  row = mysql_fetch_row(res);
  index = atoi(row[0]);
  if (length_db < 10) {
    printf("length_rank:%d\n",index);
    while (length_db > 0) {
        printf("index_query:%d\n",index);
        sprintf(query, "SELECT id, username, score, time FROM scores WHERE id=%d AND username = '%s'", index, user_name);
        execute_query(query);
        res = mysql_store_result(conn);
        row = mysql_fetch_row(res);
        msg.type = RECV_SCORE_ALONE;
        strcpy(msg.value, row[0]);
        strcat(msg.value, "|");
        strcat(msg.value, row[1]);
        strcat(msg.value, "|");
        strcat(msg.value, row[2]);
        strcat(msg.value, "|");
        strcat(msg.value, row[3]);
        send(conn_fd, &msg, sizeof(msg), 0);
        printf("row(<10):%s\t, %s\t, %s\t, %s\t\n", row[0], row[1], row[2], row[3]);
        index++;
        length_db--;
      }
  } else if (length_db >= 10) {
    while (length_db > length_db - 10) {
        sprintf(query, "SELECT id, username, score, time FROM scores WHERE id = %d AND username = '%s' ORDER BY id DESC LIMIT 10", index, user_name);
        execute_query(query);
        res = mysql_store_result(conn);
        row = mysql_fetch_row(res);
        msg.type = RECV_SCORE_ALONE;
  ...
  while ((recvBytes = recv(conn_fd, &msg, sizeof(msg), 0)) > 0)
  {
    ...
    case AUTH:
      ...
      case PLAY_ALONE:
        ...
        break;
      case SEND_SCORE_ALONE:
        printf("[%d]: %s recieved data from client.\n", conn_fd, cli->login_account);
        write_score_to_db(conn_fd, msg.value);
        break;
      case RECV_SCORE_ALONE:
        printf("[%d]: %s send data to client.\n", conn_fd, cli->login_account);
        get_score_from_db(conn_fd,cli->login_account);
        break;
      case PLAY_PVP:
